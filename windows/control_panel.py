...
class ControlPanel(QWidget):
    ...
    def apply_theme(self, tn: str):
        ...
        # Pixmap 생성 오류 해결을 위해 QSS에서 사용하는 모든 키를 매핑
        mapping = {
    ...
    def open_settings(self):
        if not hasattr(self, 'sw') or self.sw is None: self.sw = SettingsWindow(self.state, self)
...
class SettingsWindow(StyledWidget):
    def __init__(self, state, panel):
        ...
        self.tl = QLabel(); self.tl.setStyleSheet("font-size: 14px; font-weight: bold; color: white;"); lay.addWidget(self.tl)
    ...
    def save(self):
        ...
        if l != self.panel.lang.lang_code:
            ...
            if self.panel.user_name in ["나", "Me"]: self.panel.user_name = self.panel.lang.get("user_name")
            if self.state.pet_name in ["라이미", "Raimi"]: self.state.pet_name = self.panel.lang.get("pet_name")
